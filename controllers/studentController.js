  ...
  try {
    ...
    if (!email || !registerNumber || !firstName || !lastName) {
      return res.status(400).json({
        message: 'email, registerNumber, firstName and lastName are required'
      });
    }
    ...
    let finalSection = section ? String(section).trim() : '';
    ...
    if (!finalSection) {
      ...
      const sectionIndex = Math.floor(position / STUDENTS_PER_SECTION);
...
export const uploadMultipleStudents = async (req, res) => {
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const email = normalized.email || `student${Date.now()}${i}@college.edu`;

      const registerNumber =
        normalized.registernumber || normalized.regno || `REG${1000 + i}`;

      const firstName = normalized.firstname || normalized.first || `User${i}`;

      const lastName = normalized.lastname || normalized.last || `Auto${i}`;

      const department = normalized.department || '';
      const year = normalized.year || '';

      let section = normalized.section ? String(normalized.section).trim() : '';
    ...
    for (const key in group) {
      ...
      for (let i = 0; i < autoList.length; i++) {
        ...
        const sectionIndex = Math.floor(globalIndex / STUDENTS_PER_SECTION);
